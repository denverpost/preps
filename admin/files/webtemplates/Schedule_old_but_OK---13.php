...
<VAR $todayMonth = date("F",strtotime($Game_GameDate))>
<VAR $todayDate = date("j",strtotime($Game_GameDate))>
<VAR $todayYear = date("Y",strtotime($Game_GameDate))>

###<?PHP $strNow = date(Y-m-d); ?>###
###STRNOW: {$strNow}###
...
###GAMEDATE: {$Game_GameDate}<br>###
<IFGREATER $Game_GameDate $strmyBasketballTime>
<VAR $mySkedNote = "">
</IFGREATER>
</IFTRUE>
...
###SPORTNAME: {$Sport_SportName}<br>
CLASSID: {$Class_ClassID}<br>###
<IFTRUE  $Sport_SportName == "Football">
...
<IFEQUAL $Sport_SportName "Boys Basketball">
<VAR $myTerm = "Tournament">
</IFEQUAL>
<IFEQUAL $Sport_SportName "Girls Basketball">
<VAR $myTerm = "Tournament">
</IFEQUAL>
...
<IFTRUE $sportName == "Softball" || $sportName == "Boys Soccer" || $sportName == "Field Hockey" || $sportName == "Football" || $sportName == "Boys Basketball" || $sportName == "Girls Basketball" || $sportName == "Ice Hockey">
<ELSE>
...
<IFTRUE $sportName == "Boys Basketball" || $sportName == "Girls Basketball">
<?php
$confName = preg_replace("/(\w+)\s(\(\d\w\))/", "$1", "$Conference_ConferenceName");
?>
<VAR $testTime = strtotime($Game_GameDate)>
...
<VAR $Game_HomeTeamName = trim($Game_HomeTeamName)>
                ...
                {$Game_AwayTeamName}</a><IFGREATER $Game_GameStatStatus 2>: {$Game_AwayTeamPoints}
</IFGREATER>
                at
                <a href="{$externalURL}site=default&tpl=Team&TeamID={$Game_HomeTeamID}">
                {$Game_HomeTeamName}</a><IFGREATER $Game_GameStatStatus 2>: {$Game_HomeTeamPoints}
</IFGREATER>
<IFNOTEQUAL $Game_GameLocation $Game_HomeTeamID>
<IFNOTEMPTY $Game_SchoolName>
<IFNOTEMPTY $Game_SchoolAddress>
...
<?PHP } ?>
<IFTRUE $Game_GameIsConference>
<ELSE>
###<IFTRUE  $sportType == 1 || $sportType == "">
<ELSE>
({$mySkedWord})
</IFTRUE>###
<IFTRUE  $sportType == 1 || $sportType == "">
<IFTRUE $Game_GameIsTournament>
<ELSE>
({$mySkedWord})
</IFTRUE>
</IFTRUE>
